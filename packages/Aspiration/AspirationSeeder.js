/**
 * @file AspirationSeeder.js
 * @description Idempotent seeder for Aspiration module reference data (category, priority, status).
 * @domain CommunityDemocracy
 * @package Aspiration (Epic Aspiration / P70)
 */

const { AspirationConstants } = require('./AspirationEntity.js');

class AspirationSeeder {
  constructor() {
    this.db = typeof WK !== 'undefined' && typeof WK.database === 'function' ? WK.database() : null;
    this.logger = typeof WK !== 'undefined' && typeof WK.logger === 'function' ? WK.logger('AspirationSeeder') : console;
  }

  _seedLookupGroup(groupName, items) {
    if (!this.db) return;

    // Idempotent: check or create group
    let group = this.db.findOne('lookup_groups', { name: groupName });
    if (!group) {
      group = { id: `group_${groupName.toLowerCase()}`, name: groupName };
      this.db.create('lookup_groups', group);
    }

    // Idempotent: insert only missing items
    items.forEach(itemCode => {
      const existing = this.db.findOne('lookup_items', { groupId: group.id, code: itemCode });
      if (!existing) {
        this.db.create('lookup_items', {
          id: `item_${group.id}_${itemCode.toLowerCase()}`,
          groupId: group.id,
          code: itemCode,
          label: itemCode.replace(/_/g, ' '),
        });
      }
    });
  }

  run() {
    if (!this.db) return;

    // Ensure lookup base tables exist
    if (!this.db.hasTable('lookup_groups')) this.db.createTable('lookup_groups', {});
    if (!this.db.hasTable('lookup_items')) this.db.createTable('lookup_items', {});

    // Seed 3 groups from AspirationConstants
    this._seedLookupGroup('ASPIRATION_CATEGORY', AspirationConstants.ASPIRATION_CATEGORIES);
    this._seedLookupGroup('ASPIRATION_PRIORITY', AspirationConstants.ASPIRATION_PRIORITIES);
    this._seedLookupGroup('ASPIRATION_STATUS', AspirationConstants.ASPIRATION_STATUSES);

    if (this.logger && this.logger.info) {
      this.logger.info('Aspiration lookup reference data seeded successfully (CATEGORY, PRIORITY, STATUS).');
    }
  }

  static isSeeded() {
    const db = typeof WK !== 'undefined' && typeof WK.database === 'function' ? WK.database() : null;
    if (!db || !db.hasTable('lookup_groups')) return false;
    return !!db.findOne('lookup_groups', { name: 'ASPIRATION_CATEGORY' });
  }

  static hasData() {
    const db = typeof WK !== 'undefined' && typeof WK.database === 'function' ? WK.database() : null;
    if (!db || !db.hasTable('aspirations')) return false;
    const records = db.search('aspirations', {});
    return records && records.length > 0;
  }
}

module.exports = { AspirationSeeder };

